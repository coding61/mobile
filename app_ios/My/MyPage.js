...
var ImagePicker = require('react-native-image-picker');    //图片库
var qiniu = NativeModules.UpLoad;                          //导入原生的七牛上传
var RNBridgeModule = NativeModules.RNBridgeModule;         //导入原生融云
...
class MyPage extends Component {
	...
	// 上传图片到七牛空间
	_uploadAvatarToQiniu(filename, token, key){
		var that = this;
		qiniu.uploadImage(filename, token, key, (error, callBackEvents)=>{
			if (error) {
				...
				if (callBackEvents.url) {
					// 开始上传修改服务器存储的头像
					that._uploadAvatar(callBackEvents.url);
				}else{
					// 关闭动画
					that.hideLoading();
	...
	_uploadAvatar(avatar){
		this._fetchUpdateInfo("avatar", "", avatar);
	}
  	render() {
    	return (
