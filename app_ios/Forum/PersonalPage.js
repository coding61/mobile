...
class PersonalPage extends Component {
  ...
  onPress(num) {
  	switch (num) {
  	    case 0: {
            this.props.navigation.navigate('MyForum', this.state.userinfo.owner);
            break;
        }
        case 1: {
    ...
    return (
      <View style={{flex: 1, backgroundColor: 'rgb(243, 243, 243)'}}>
      		...
      		<View style={{backgroundColor: 'rgba(255, 255, 255, 0)', width: width, height: width * 485 / 750, alignItems: 'center', justifyContent: 'center'}}>
                ...
                <TouchableOpacity style={{width: width * 170 / 750, height: width * 170 / 750, marginTop: 50}}>
                ...
                <Text style={{color: 'white', fontSize: 18, marginTop: 20}}>{this.state.userinfo.name}</Text>
                <View style={{marginTop: 20, flexDirection: 'row', justifyContent: 'center'}}>
                    <Text style={{color: 'white', fontSize: 18}}>{'段位:' + this.state.userinfo.grade.current_name}</Text>
                    <Text style={{color: 'white', fontSize: 18, marginBottom: 3, paddingLeft: 10, paddingRight: 10}}>{'  |  '}</Text>
                    <Text style={{color: 'white', fontSize: 18}}>{'钻石:' + this.state.userinfo.diamond}</Text>
                </View>
      		</View>
  			<View style={{marginTop: 20}}>
      			<TouchableOpacity onPress={this.onPress.bind(this, 0)} style={{flexDirection: 'row', alignItems: 'center', borderColor: 'rgb(238, 238, 239)', borderTopWidth: 1, width: width, height: 50, backgroundColor: 'white'}}>
      				...
      				<Text style={{marginLeft: 15, fontSize: 15, color: 'rgb(59, 60, 61)'}}>{'她的帖子'}</Text>
