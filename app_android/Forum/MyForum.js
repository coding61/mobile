...
export default class MyForum extends Component{
    ...
    // 获取他人帖子列表
    _fetchOtherForumList(pagenum, dataSource, callback){
        Utils.isLogin((token)=>{
            if (token) {
                var type = "get",
                    url = Http.otherForumList(pagenum, this.props.navigation.state.params.owner),
                    token = token,
                    data = null;
                BCFetchRequest.fetchData(type, url, token, data, (response) => {
                    // console.log(response.results);
    ...
    // 帖子详情
    forumdetail(data){
        this.props.navigation.navigate('Forum_Details', { data: data.pk, iscollect:data.collect,name:'my',callback:(msg)=>{
            //刷新页面
            this.reloadPage();
        }})
    }
    ...
    render(){
        return (
            <View style={{flex:1}}>
                <BCFlatListView 
                    ref="bcFlatlist"
                    fetchData={this.props.navigation.state.params.flag=='我的帖子'?this._fetchForumList.bind(this):this._fetchOtherForumList.bind(this)} 
